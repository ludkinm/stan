#ifndef STAN_SERVICES_SAMPLE_HUGNHOP_DIAG_E_HPP
#define STAN_SERVICES_SAMPLE_HUGNHOP_DIAG_E_HPP
...
#include <stan/mcmc/hugnhop/diag_e_hugnhop.hpp>
...
int hugnhop_diag_e(Model& model, const stan::io::var_context& init,
  ...
  stan::mcmc::diag_e_hugnhop<Model, boost::ecuyer1988> sampler(model, rng);

  logger.info("Made a sampler. Setting metrics etc.");
...
int hugnhop_diag_e(Model& model, const stan::io::var_context& init,
  ...
  return hugnhop_diag_e(model, init, unit_e_metric, random_seed, chain,
                        init_radius, num_warmup, num_samples, num_thin,
                        save_warmup, refresh, stepsize, stepsize_jitter,
                        int_time, lambda, kappa, interrupt, logger, init_writer,
                        sample_writer, diagnostic_writer);
}
