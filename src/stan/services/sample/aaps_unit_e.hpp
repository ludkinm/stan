#ifndef STAN_SERVICES_SAMPLE_AAPS_UNIT_E_HPP
#define STAN_SERVICES_SAMPLE_AAPS_UNIT_E_HPP
...
int aaps_unit_e(Model& model, const stan::io::var_context& init,
                unsigned int random_seed, unsigned int chain,
                double init_radius, int num_warmup, int num_samples,
                int num_thin, bool save_warmup, int refresh, double stepsize,
                double stepsize_jitter, callbacks::interrupt& interrupt,
                callbacks::logger& logger, callbacks::writer& init_writer,
                callbacks::writer& sample_writer,
                callbacks::writer& diagnostic_writer) {
  std::cout << "Called aaps_unit_e service: " << std::endl;
  ...
  stan::mcmc::unit_e_aaps<Model, boost::ecuyer1988> sampler(model, rng);
  sampler.set_nominal_stepsize(stepsize);
  sampler.set_stepsize_jitter(stepsize_jitter);

  std::cout << "AAPS going to run sampler: " << std::endl;
