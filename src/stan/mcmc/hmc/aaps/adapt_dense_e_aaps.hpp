#ifndef STAN_MCMC_HMC_AAPS_ADAPT_DENSE_E_AAPS_HPP
#define STAN_MCMC_HMC_AAPS_ADAPT_DENSE_E_AAPS_HPP

#include <stan/callbacks/logger.hpp>
#include <stan/mcmc/hmc/aaps/dense_e_aaps.hpp>
...
class adapt_dense_e_aaps : public dense_e_aaps<Model, BaseRNG>,
                           public stepsize_covar_adapter {
 public:
  adapt_dense_e_aaps(const Model& model, BaseRNG& rng)
      : dense_e_aaps<Model, BaseRNG>(model, rng),
        stepsize_covar_adapter(model.num_params_r()) {}

  ~adapt_dense_e_aaps() {}

  sample transition(sample& init_sample, callbacks::logger& logger) {
    sample s = dense_e_aaps<Model, BaseRNG>::transition(init_sample, logger);
