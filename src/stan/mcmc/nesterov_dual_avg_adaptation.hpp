#ifndef STAN_MCMC_NESTEROV_ADAPTATION_HPP
#define STAN_MCMC_NESTEROV_ADAPTATION_HPP
...
class nesterov_adaptation : public base_adaptation {
 public:
  nesterov_adaptation()
  ...
  void learn_log_param(double& param, double adapt_stat) {
    ++counter_;

    adapt_stat = adapt_stat > 1 ? 1 : adapt_stat;

    // Nesterov Dual-Averaging of log(param)
  ...
  void complete_adaptation(double& param) { param = std::exp(x_bar_); }
